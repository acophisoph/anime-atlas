...
export function Header({
  ...
  atlasMode: 'media' | 'people';
  setAtlasMode: (m: 'media' | 'people') => void;
}) {
  return (
    <header style={{ display: 'flex', gap: 8, alignItems: 'center', padding: 10, borderBottom: '1px solid #333' }}>
      ...
      <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
        ...
        {(['ANIME', 'MANGA', 'BOTH'] as Scope[]).map((s) => (
          <button key={s} onClick={() => setScope(s)} disabled={scope === s}>
            {t.scope[s]}
          </button>
        ))}
        <button onClick={() => setAtlasMode('media')} disabled={atlasMode === 'media'}>
          Media Atlas
        </button>
        <button onClick={() => setAtlasMode('people')} disabled={atlasMode === 'people'}>
          Staff Atlas
        </button>
      </div>
    </header>
